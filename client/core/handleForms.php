...
if (isset($_POST['insertOfferBtn'])) {
	...
	// Check if user can submit offers (client or administrator)
	if (!$userObj->canSubmitOffers()) {
		$_SESSION['status'] = '400';
		$_SESSION['message'] = "You don't have permission to submit offers!";
		header("Location: ../index.php");
		exit;
	}
	
	// CHECKS IF OFFER ALREADY EXISTS 
	//NEW ADDITION	- 13/09
	if($offerObj->offerExists($user_id, $proposal_id)) {
		$_SESSION['status'] = '400';
		$_SESSION['message'] = "Offer already exists!";
		header("Location: ../index.php");
		exit;
	}
